...
# Usage:
#   cd <your-project>
#   sh venv_setup/venv-linux.sh              # create local venv
#   sh venv_setup/venv-linux.sh symlink      # symlink to Manager's ../venv (Linux only)
#
...
if [ $# -eq 0 ]; then
    echo ""
    echo "Usage:  sh venv_setup/venv-linux.sh [symlink | go]"
    echo ""
    echo "  symlink  Preferred (Linux): creates venv as a symlink to the Manager's"
    ...
    echo "           Use this if the project is not inside the Manager folder."
    echo ""
    exit 0
fi
