...
**Update 2026-08-23:** Option A implemented (`TRANS_UPDATE_LOCKING`, LogicBank v1.33.00) and Tier 1/1.5 tests built (`examples/concurrent_adjust/`) — see §6 for the corrected SQLite finding discovered while building the Tier 1 test (SQLite gets a real, verified improvement from `pessimistic`, not "zero improvement" as originally stated).
...
**SQLite caveat — corrected after building the Tier 1 regression test (see §6 testing plan below):** `with_for_update()`'s `FOR UPDATE` clause is confirmed dropped by SQLAlchemy's SQLite dialect (verified directly — `test_for_update_sql.py`). But the implementation pairs `with_for_update()` with `populate_existing()` (needed regardless, per the note below about already-cached parents), and `populate_existing()` is *not* a no-op on SQLite — it forces a fresh read that overwrites a session's stale, already-cached copy of the parent row. Built live (`examples/concurrent_adjust/tests/test_interleaved_race.py`): the exact two-session interleaving from §3 (both sessions read `balance=900` before either writes) is **correctly rejected** under `pessimistic` on SQLite — `session2.commit()` raises `ConstraintException` instead of silently persisting `980`. This is a real, verified improvement over `ignored` on SQLite, not a no-op — but it is **not equivalent to genuine row locking**: it only closes the race for two sessions in the *same process*, sharing the same SQLite connection, executing *sequentially* (one full commit completing before the other session's commit begins) — not two genuinely concurrent connections/processes. The real value of the *locking* half of this fix (blocking a second writer that arrives mid-transaction) remains Postgres/MySQL/SQL Server/Oracle-only; the *fresh-read* half (`populate_existing()`) benefits SQLite too, for the same-process/sequential-session shape most SQLite deployments (single dev process, `basic_demo`, tutorials) actually exhibit.
...
**SQLite is not fully moot — corrected, see the §6 Option A SQLite caveat above.** `with_for_update()`'s lock is a no-op under SQLite, but the `populate_existing()` this implementation pairs with it is not: it closes the exact same-process/sequential-session interleaving in `examples/concurrent_adjust/tests/test_interleaved_race.py`, including for `basic_demo` and most sample/tutorial projects (which are typically single-process, sequential-request patterns, not genuinely concurrent connections). It does **not** protect against two truly concurrent SQLite connections/processes racing each other mid-transaction — that shape still relies on SQLite's own accidental file-locking behavior in §5, unchanged. State this distinction plainly wherever the setting is documented: `pessimistic` measurably helps SQLite deployments in the common case, but is not a substitute for genuine row locking there.
...
**Testing strategy — built, per the tiered plan below, kept intentionally lightweight to match LogicBank's existing test suite (fast, self-contained, no Docker, no live external DB). Fixture: `examples/concurrent_adjust/` (Customer/Order, matching §3's flagship scenario exactly), structured after `examples/multi_relns/` (the closest existing model — entirely single-session `unittest`, SQLite-backed, no threads).**

- **Tier 1 — built (`tests/test_interleaved_race.py`, SQLite, deterministic, no threads):** two ordinary SQLAlchemy sessions against the same SQLite DB, sequenced deliberately by the test (both read `Customer.balance=900` before either writes; session 1 commits an `Order` of 80; session 2 then commits a second `Order` of 80). No thread, no sleep, no timing luck — the test controls the interleaving explicitly. Two test methods:
  - `test_concurrent_orders_under_ignored_locking_silently_exceed_credit_limit` — confirms the bug reproduces exactly as documented under `ignored` (today's default): session 2's commit succeeds, persisting `980` instead of correctly rejecting a true combined total of `1060` against `credit_limit=1000`.
  - `test_pessimistic_locking_closes_the_race_even_on_sqlite` — see the corrected SQLite finding above; confirms session 2's commit now correctly raises `ConstraintException` under `pessimistic`, even on SQLite.

- **Tier 1.5 — built (`tests/test_for_update_sql.py`, pure SQL-compilation check, no DB connection at all):** confirms `_get_parent_logic_row`'s query — `with_for_update().populate_existing()` — compiles to include a literal `FOR UPDATE` clause against `sqlalchemy.dialects.postgresql.dialect()`, and confirms the same query compiles with the clause silently dropped against `sqlalchemy.dialects.sqlite.dialect()`. Both directions asserted, so a future SQLAlchemy version changing either dialect's behavior is caught as a test failure, not silently assumed. No engine connection, no live Postgres, milliseconds to run — proves LogicBank *asks* the DB to lock, without needing to prove Postgres *honors* the ask (that's SQLAlchemy/Postgres's own well-established contract).

- **Tier 2 — documented, not built:** a live-Postgres test with two real threads, synchronized so the test can assert thread B is genuinely blocked inside `with_for_update()` until thread A commits — the only way to prove the lock *blocks* two genuinely concurrent connections, not just that it's requested, and the only tier that would validate real cross-connection locking rather than the same-process/sequential-session shape Tier 1 exercises. Deliberately not building this: it requires Docker/a live Postgres instance and real thread synchronization, a materially different (heavier, flakier-if-done-wrong) footprint than anything else in LB's test suite today, for a claim (SQLAlchemy's `FOR UPDATE` actually blocks concurrent readers under Postgres) that is Postgres's own documented, long-established behavior — not a novel claim this codebase is introducing. If this is ever contested or a regression is suspected specifically in the blocking behavior, build Tier 2 then, informed by the specific doubt — not speculatively now.
