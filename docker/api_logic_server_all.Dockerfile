# GA release
# docker buildx build --push -f docker/api_logic_server_all.Dockerfile --tag apilogicserver/api_logic_server_all:9.01.12 -o type=image --platform=linux/arm64,linux/amd64 .

# Beta - test codespaces with tutorial, API_Fiddle (change .devcontainer.json -> apilogicserver/api_logic_server_all_x)
# docker buildx build --push -f docker/api_logic_server_all_x.Dockerfile --tag apilogicserver/api_logic_server_all_x:9.01.12 -o type=image --platform=linux/arm64,linux/amd64 .
...
# docker run -it --name api_logic_server --rm --net dev-network -p 5656:5656 -p 5002:5002 -v ${PWD}:/localhost apilogicserver/api_logic_server_all
...
# docker buildx create --name mybuilder;  docker buildx use mybuilder   # first time only setup
#    delete: docker buildx rm mybuilder; docker buildx ls

# docker buildx build -f docker/api_logic_server_all.Dockerfile --tag apilogicserver/api_logic_server_all -o type=image --platform=linux/arm64,linux/amd64 .
