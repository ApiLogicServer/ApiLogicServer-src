...
if [ $# -eq 0 ]
  then
    ...
    echo "   > sh docker/webgenie_docker/build_web_genie_x.sh [local | deploy]"
    echo " "
...
if [ "$1" = "local" ]
  then
    ...
    docker buildx build --push -f webgenie.Dockerfile --tag apilogicserver/web_genai_x:14.03.21 -o type=image --platform=linux/arm64,linux/amd64 .
    docker buildx build --push -f webgenie.Dockerfile --tag apilogicserver/web_genai_x:latest -o type=image --platform=linux/arm64,linux/amd64 .
fi
