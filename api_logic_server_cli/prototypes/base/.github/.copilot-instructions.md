...
## What is this?
This is Context Engineering for GenAI-Logic Projects (created into each project) - version 3.11.
...
## 🔑 Core Architectural Principles (Always Apply)

**Data Models:**
- ✅ **Always use autoincrement:** `id = Column(Integer, primary_key=True, autoincrement=True)`
- ✅ **Create columns for derived values:** If logic calculates `balance`, add `balance` column to model

**Business Logic:**
- ✅ **Try HARD to use rules first:** Formulas, sums, counts, constraints
- ✅ **Fall back to events only when rules can't:** Audit trails, external integrations, side effects
- ✅ **Calculations in formulas, not events:** Even complex conditionals (e.g., HST vs GST+PST)

**Complex Inserts/Integrations:**
- ✅ **Request Pattern over Custom API:** Lookups + validation + insert? → SysXxxReq table + before_event, not custom API endpoint

**For detailed subsystem creation guidance:** Ask to read `docs/training/subsystem_creation.md` when creating multiple tables with business logic
...
## 🎯 Logic Patterns Quick Reference

**WHEN USER ASKS: "what is ROP" or "what is Request Pattern"**

**SHORT ANSWER (30 seconds):**

The **Request Pattern (ROP)** is a table design for integration services:

- **Request fields** = user input (e.g., product_id, hs_code_id, value_amount)
- **Response fields** = system output (e.g., chosen_supplier_id, duty_amount, reason)
- **early_row_event** = performs integration (AI calls, external APIs, calculations)

**Pattern**: Insert row with request fields → early_row_event runs integration service → populates response fields → automatic audit trail

**Use for**: AI decisions, email/messaging, external API calls, complex calculations requiring audit

**Example**: AI supplier selection - insert SysSupplierReq(product_id=1) → early_row_event calls AI → populates chosen_supplier_id, chosen_unit_price, reason

**Full details**: See `docs/training/RequestObjectPattern.md` (comprehensive guide with examples)

---

**WHEN USER PROVIDES PROMPT LIKE:**
- "calculate/determine/select [X] when [Y] is given"
- "call external service for [purpose]"
- Integration service needs (AI, messaging, pricing, routing)
- Compliance/audit domain (customs, finance, healthcare)

**IMMEDIATELY SUGGEST REQUEST PATTERN:**
1. Create table with request fields (input) + response fields (output)
2. Implement early_row_event for integration service
3. Make API thin wrapper (just inserts request fields)
4. Optional: Create wrapper function to hide complexity

**WHY**: Single source of truth, works from any entry point, automatic audit, testable, governed by deterministic rules

**DON'T**: Put business logic in API services (creates duplication, bypasses rules engine governance)
