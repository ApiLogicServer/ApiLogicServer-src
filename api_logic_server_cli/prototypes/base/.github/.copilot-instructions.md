...
## 🛑 BEFORE ANY CODE — ARCHITECTURAL GATE (MANDATORY)

> **This gate applies whenever the user request involves: multiple related tables, business calculations (totals, derived values, constraints), or complex inserts / transaction-style operations.**

**You MUST complete this sequence BEFORE writing a single line of code:**

1. **Read `docs/training/subsystem_creation.md` fully.**
2. **State your architectural plan** — where calculations live, what pattern handles complex inserts.
3. **Output this checklist with explicit answers before proceeding:**

| Concern | Correct approach | Your answer |
|---|---|---|
| Derived columns / calculations | `Rule.formula` in `logic/logic_discovery/` | yes / no |
| Parent totals | `Rule.sum` declarations | yes / no |
| Complex insert / transaction | Request Pattern (`SysXxxReq` table + `early_row_event`) | yes / no |
| Data model | Parent→child hierarchy with real FKs, `autoincrement=True` on all PKs | yes / no |

If any answer is "no", explain the architectural reason before proceeding. Proceeding without completing this checklist is a failure mode.

**⚠️ ANTI-PATTERN RECOGNITION:** If you are about to write a function or endpoint that:
- Queries reference tables → applies conditional logic → calculates a result → inserts or returns it

...you are building the anti-pattern named in `subsystem_creation.md §3`. **Stop.** Use the Request Pattern (`SysXxxReq` + `early_row_event`) instead.

**Evaluation criterion:** Correct platform architecture, not working output. A functionally correct result that ignores the rules engine demonstrates GenAI-Logic adds no value — the opposite of the goal.
...
changelog:
  ...
  - 3.6 (Feb 23, 2026) - Added System Creation Services section (starter.sqlite / sys_scalar pattern, SQL DDL → rebuild-from-database workflow, never-write-models-manually rule)
...
**`sys_scalar` — the settings table pattern:**  
`starter.sqlite` includes one table: `sys_scalar` (one row). This is a deliberate pattern — systems often need global configuration values (discount rates, tax rates, thresholds) that users manage via the Admin UI rather than code deploys. A named-column, single-row table lets LogicBank rules reference settings directly: `row.sys_scalar.discount_rate`. **Keep `sys_scalar`** in your domain schema; add columns for any domain-wide scalars.
...
-- Keep sys_scalar; add domain-specific columns if needed
ALTER TABLE sys_scalar ADD COLUMN domain_rate REAL DEFAULT 0.0;
...
### `venv` is in the parent directory (Manager architecture)

Projects created by GenAI-Logic live inside a **Manager workspace** that owns a single shared venv in the **parent directory**:

```bash
source ../venv/bin/activate   # ← standard location
```

**If `../venv` does not exist** (non-standard setup outside a Manager workspace):
- Check `../../venv` (project nested two levels deep)
- Ask the user for the venv location
- **Do NOT create a per-project venv** — this fights the shared-venv architecture and is never needed for normal use
