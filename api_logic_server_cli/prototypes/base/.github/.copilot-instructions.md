...
4. **Extract domain constants and FK relationships first, then design schema as SQL DDL:**
   ...
   **SQLAlchemy mapper args** (added by `rebuild-from-database` when it detects the pattern, or add manually):
   ```python
   class Employee(Base):
   ...
   **LogicBank rules — scope by class:**
   ```python
   # Fires for ALL employees:
   Rule.formula(derive=models.Employee.some_shared_column, ...)

   # Fires only for hourly employees — can reference both Employee and HourlyEmployee columns:
   Rule.formula(derive=models.HourlyEmployee.weekly_pay,
                as_expression=lambda row: row.hours_worked * row.hourly_rate)

   Rule.constraint(validate=models.HourlyEmployee,
                   as_condition=lambda row: row.weekly_pay <= row.max_weekly_pay,
                   error_msg="Weekly pay ({row.weekly_pay}) exceeds cap ({row.max_weekly_pay})")
   ```

   **Key rule:** if a parent table needs to aggregate across ALL subtypes (e.g. `Department.total_payroll = sum of Employee.weekly_pay`), the aggregated column (`weekly_pay`) must live on the **base** `employee` table — not only on a subtype — so LogicBank can see it for every row.
