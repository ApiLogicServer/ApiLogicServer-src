...
version: 2.8
changelog:
  - 2.8 (Nov 16, 2025) - Probabalistic Logic, keep training generic
...
- ✅ **Code templates** → `template_probabilistic_rules.py`
...
**Training File Hierarchy (Read in Order):**
...
3. **`docs/training/logic_bank_api_probabilistic.prompt`** - Probabilistic (AI) rules API
   - compute_ai_value() utility for AI-driven value computation
...
**When generating ANY rule code:** Always start with logic_bank_patterns.prompt for foundational patterns, then use the appropriate API file. Probabilistic rules work seamlessly with deterministic rules.
...
**CRITICAL: Database reorganization workflow**

⚠️ **MANDATORY DATABASE VERIFICATION BEFORE GENERATING LOGIC**:

When AI logic generation needs new tables (e.g., SysSupplierReq for probabilistic rules):

1. **VERIFY TABLE EXISTS IN DATABASE FIRST** (not just models.py):
   ```bash
   sqlite3 database/db.sqlite ".tables" | grep -i table_name
   ```
   - ❌ WRONG: Assuming table exists because model is in models.py
   - ✅ CORRECT: Verify actual database table before generating logic

2. **IF TABLE MISSING** - STOP AND TELL USER:
   ```
   CRITICAL: The [TableName] table does not exist in the database.
   
   I cannot run Alembic migrations directly because:
   - Virtual environment location varies per system
   - Migration commands require proper Python environment
   
   YOU must run these commands:
   
   cd database
   alembic revision --autogenerate -m "Add [TableName]"
   alembic upgrade head
   
   Then confirm the migration succeeded before I continue generating logic.
   ```
   
   ❌ DO NOT try to run alembic commands yourself (venv location unknown)
   ❌ DO NOT generate logic files that reference non-existent tables
   ❌ DO NOT proceed until user confirms migration completed
   ✅ DO provide clear instructions for user to run the migration

3. **AFTER USER CONFIRMS MIGRATION** - Then generate:
   - Logic files with rules and event handlers
   - UI configuration (admin.yaml updates)
