...
3. **Check for Request Pattern signals** — apply ONLY for integration side-effects: AI calls, email, Kafka, external APIs. (`docs/training/RequestObjectPattern.md`)
   ...
   **🚨 EXISTING-SCHEMA SCAN — applies to EVERY new requirement, including ones added
   incrementally after the initial build:** Before designing any new column, run
   `.tables` / `.schema` on `database/db.sqlite` (or scan `database/models.py`) for
   tables that already represent the entity the requirement talks about. A requirement
   phrased as "X should not be able to Y if Z" where Z is a count/state of related rows
   is a `Rule.count` + `Rule.constraint` derivation, NOT a manually-set integer column.

   | Requirement phrase | Likely existing table | Wrong approach | Right approach |
   |---|---|---|---|
   | "unresolved past due letters" | `SysEmail` (already created by an earlier email requirement) | Add `Customer.past_due_letter_count` as a manually-set integer | Add `SysEmail.resolved`, then `Rule.count(derive=Customer.past_due_letter_count, as_count_of=SysEmail, where=lambda row: row.resolved == False)` + `Rule.constraint` |

   **Real failure case:** for "Customers should not be able to create new orders if they
   have unresolved past due letters", the first attempt added a bare
   `past_due_letter_count` column with no derivation — nothing kept it in sync. The
   correct design reuses `SysEmail` (already present from the email/Request Pattern
   step), adds a `resolved` flag, and derives the count with `Rule.count`.
