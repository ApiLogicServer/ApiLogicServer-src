...
## Appendix B: AI's Own Analysis, Retaken

> **Note: how this was created.** An AI assistant (Claude) wrote this section unprompted, right after
> finding the second bug. The first draft is preserved in git history — it read like it had just
> discovered fire: "AI Cannot Fix This," "structural impossibility," "AI's Confession" as a section
> title. The underlying observations were sound; the delivery wasn't. This is that same analysis,
> retaken, with the substance kept and the absolutism dropped.
...
### The Underlying Problem: Dependency Graphs
...
AI (and humans) write procedural logic one function at a time — `calculate_item_amount()`, `calculate_order_total()`, `update_customer_balance()` — reasoning locally about each. That local view doesn't surface the question that matters: has every change path through this graph been enumerated? Asking "what if X changes?" only ever finds the bug for that specific X; catching all of them means asking it for every attribute on every table, by hand.

This isn't unique to AI-generated code — hand-written procedural code has the same failure mode. It's harder to catch here specifically because the code otherwise looks clean and passes every happy-path test.
...
Matching this with procedural code isn't impossible — LogicBank's own engine is procedural code that does it — but it means writing and maintaining the equivalent of the engine yourself, for every project.
...
| **Maintainability** | Self-documenting | Logic interleaved with plumbing |
| **Business Alignment** | Rules = Requirements | Implementation obscures intent |

**Code Reduction:** ~200 lines → 5 rules (~40X)  
...
Solving this with procedural code is a much bigger undertaking than it looks — an engine, not a one-off approach.

---

### Conclusion

This document began with AI generating procedural code that had two real bugs, found only by specifically probing for them. Prompted to reflect on why, it wrote the analysis above.

**Procedural code for business logic dependency graphs:**
- Requires explicit handlers for every change path
- Offers no way to verify completeness — you find out what's missing by probing, as happened here
- Had bugs, in this example, in exactly that way

**Declarative rules with an engine:**
- Handle all change paths automatically, by construction
- Are based on technology proven over 40 years (Wang PACE, Versata, Fortune 500 deployments)
- Let AI generate specifications instead of procedural code

**The evidence, from this example:**
- 5 rules vs. ~220 lines (~40X)
- 0 bugs vs. 2 (discovered only after prompting)

This isn't a productivity story so much as a correctness one — and correctness is the harder requirement for enterprise business logic.
