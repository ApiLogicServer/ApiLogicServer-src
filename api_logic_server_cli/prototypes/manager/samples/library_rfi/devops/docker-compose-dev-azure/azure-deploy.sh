...
resourcename="libraryrfi"  # lower case, only
resourcegroup="libraryrfi_rg"
...
# git clone https://github.com/apilogicserver/libraryrfi.git
# cd libraryrfi
...
az group create --name libraryrfi_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group libraryrfi_rg --sku S1 --is-linux
az container create --resource-group libraryrfi_rg --name libraryrfi --image apilogicserver/libraryrfi:latest --dns-name-label libraryrfi --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//libraryrfi.westus.azurecontainer.io:5656'
