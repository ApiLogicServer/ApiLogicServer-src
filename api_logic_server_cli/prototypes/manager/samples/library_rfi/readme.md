<!--
  ...
  Version: 3.15  (Aug 5 2026) — Section 4 now points at docs/requirements/
    project_creation_report.md and project_creation_prompt.md (generated by
    create_provenance.py at `create` time), replacing a stale
    Behave-report-only forward reference.
...
!!! pied-piper ":bulb: TL;DR - Requirements from Interview, on a domain that isn't customer/orders"

    Created by: 💬 *Create a system called library_rfi for a small public library — tracking members, books, and loans. Interview me to work out the rules: how loans and fines should work, and what should happen when a member wants a book that's already checked out.*

    See [the full transcript](docs/requirements/library_rfi-transcript.md).

    * Nearly the entire spec came from the interview, not the prompt — a deliberately minimal starting point
    * Same confirmed requirements, restated two more ways for comparison: [as Gherkin](docs/requirements/library_gherkin.md), [as procedural steps](docs/requirements/library_procedural.md)
    * A second, independent build from the procedural version converged on the same declarative rule patterns
...
**How this project was created:** see
[`docs/requirements/project_creation_report.md`](docs/requirements/project_creation_report.md)
— scaffold source, creation date, and a running index of every use case implemented
(each linking to its own `ad-libs.md`, listing decisions made beyond the spec). See
[`docs/requirements/project_creation_prompt.md`](docs/requirements/project_creation_prompt.md)
for what was originally requested.

Optionally, you can also **document requirements** as part of an **executable test plan**.  Test plan execution creates documentation (in markdown), including **requirements traceability** into implementation.  [See example here](test/api_logic_server_behave/reports/Behave%20Logic%20Report%20Sample.md).
