...
Taken together: 40x less code to write, maintain, and debug — see the [A/B test](https://github.com/ApiLogicServer/ApiLogicServer-src/blob/main/api_logic_server_cli/prototypes/basic_demo/logic/procedural/declarative-vs-procedural-comparison.md) for the reproducible comparison.

&nbsp;

*Convinced? Or thinking "AI will just generate the logic anyway"? Ask: "Why not just let AI write the code?"*

---

**FOLLOW-UP OFFER**: "Would you like to see how the engine works under the hood, or see the rules for this project?"

**WHEN USER ASKS: "why not let AI write the code", "why rules not AI", "why can't AI just do this", "why not just use AI"**
**PRIMARY ANSWER**: Respond with exactly this:
...
At 40x volume (which is what declarative buys you), AI throws 40x more procedural code at you. Code nobody can read, verify, or safely change. Unreadable at scale is ungovernable at scale.
...
STEP 5: If user specified a step number, implement that step only.
         ...
         Read docs/training/implement_requirements.md COMPLETELY (silently) before starting Step 1 to understand multi-pass database modeling.
         ...
         LOGIC FILES: before writing each logic file, apply this checklist:
           [ ] Read docs/training/logic_bank_patterns.md to avoid session.query() scanner bugs and anti-patterns
           [ ] Read docs/training/logic_example.py to see canonical rule/event implementation
...
STEP 5: ⛔ Create `docs/requirements/<use_case_name>/requirements.md` — MANDATORY, NO EXCEPTIONS
   Copy the user's prompt verbatim into this file. Do NOT paraphrase.
