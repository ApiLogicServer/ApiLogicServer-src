...
## What is this?
This is Context Engineering for GenAI-Logic Projects (created into each project) - version 3.9.
...
1. **Add business logic** - Describe requirements in natural language, I'll generate declarative rules (Business Rules + AI Rules)
...
12. **Critical patterns** - Request Pattern for AI calls, email, Kafka, and audit trails
...
After seeing "GenAI-Logic" and "AI" mentioned throughout, you're probably wondering: what's
actually AI and what's the product?

The Four Layers (Plus AI Roles)
...
- **AI role:** Optional (can generate test data, schemas from natural language)
...
- **AI role:** ❌ NONE (deterministic runtime, not AI)
- **Runs in production:** ✅ YES (this is the core product)
...
- **AI role:** Claude/Copilot reads this to help YOU write code
- **Runs in production:** ❌ NO (documentation for AI, not runtime)

Layer 3A: AI Codegen (Development-Time Assistant)
- Claude/Copilot reads Layer 2 → generates code for Layers 0-1
- Converts natural language → declarative rules
- **Example:** You say "balance = sum of orders" → AI writes Rule.sum(...)
- **AI role:** Translates business requirements into declarative rules
- **Human-in-the-loop:** AI suggests code, developers review and approve
- **Runs in production:** ❌ NO (development-time only)

Layer 3B: AI Rules (Runtime - Optional)
- AI-driven decisions DURING transactions
- **Example:** Placing order → AI selects optimal supplier based on cost, lead time, world conditions
- **Implementation:** Request Pattern - insert SysSupplierReq → event calls OpenAI → populates result
- **Fallback handling:** If AI unavailable, uses deterministic fallback (e.g., min cost)
- **AI role:** Intelligent selection (supplier optimization, dynamic pricing, route selection)
- **Runs in production:** ✅ YES (optional, inside deterministic guardrails)

The Key Distinctions

What's AI:
- **Layer 3A:** AI helps you WRITE the rules (development-time)
- **Layer 3B:** AI makes DECISIONS within rules (runtime, optional)

What's NOT AI:
- **Layer 0:** Infrastructure generation (templates, introspection)
- **Layer 1:** Rules engine (deterministic enforcement)
- **Layer 2:** Documentation (static training materials)

vs. "Just AI Codegen" (like Cursor alone):
- AI codegen: Generates code you maintain manually
- GenAI-Logic: Generates declarative config the engine executes + optional runtime AI decisions

vs. "Just Infrastructure" (like Rails, Hasura):
- Others: Generate APIs, stop there
- GenAI-Logic: Generates APIs + logic enforcement + AI assistance + optional AI rules

vs. "Vibe for Backends":
- Vibe: Generates static code you own
- GenAI-Logic: Generates infrastructure + active runtime enforcing correctness + optional AI decisions
...
5. AI can optionally make runtime decisions (Layer 3B)
...
- ✅ AI Rules (Layer 3B) - Optional runtime probabilistic reasoning
- ✅ Docker containers

Does NOT run in production:
- ❌ AI assistants for codegen (Layer 3A)
- ❌ Context Engineering materials (Layer 2)
- ❌ Code generation process

The product is the engine. The AI helps you configure it AND (optionally) makes intelligent decisions within it.
