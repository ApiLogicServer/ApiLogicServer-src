...
**🚨 PITFALL — Cross-file event ordering: `early_row_event` vs `Allocate` on the same class**

`Allocate` is internally a subclass of `EarlyRowEvent`. LogicBank fires early events in registration
order (plain list append). `auto_discovery` loads files via `os.walk()`, whose order is **not
guaranteed** across files. If your AI fuzzy-match event and your `Allocate` are in separate files,
`Allocate` may be registered first and fire before the AI has set `project_id` — silently returning
an empty recipient list and allocating nothing.

**Two ordering facts to know:**
- **Within a single `declare_logic()`:** registration order = declaration order (guaranteed).
- **Across files:** order is determined by `os.walk()` and is non-deterministic.

**Fix:** Import the AI handler into the Allocate file and declare its `early_row_event` as the
**first statement** of `declare_logic()`, before any `Allocate(...)` call:

```python
# In charge_distribution.py — import handler from the AI file
from logic.logic_discovery.ai_requests.project_identification import identify_project_for_charge

def declare_logic():
    # ── AI project identification — registered FIRST so it fires before Allocate ──
    # Do NOT rely on auto_discovery file order for this — declare explicitly here.
    Rule.early_row_event(on_class=models.Charge, calling=identify_project_for_charge)

    # ... companion rules ...

    # Level-1 Allocate fires AFTER the AI event above (same declare_logic, later in list)
    Allocate(provider=models.Charge, ...)
```

The `project_identification.py` file's own `declare_logic()` only registers the `SysProjectReq`
handler (a different class) — no ordering conflict there.

---

**🚨 PITFALL — SQLAlchemy relationship proxy not refreshed after programmatic FK set**

When an `early_row_event` sets `row.project_id = matched_id`, SQLAlchemy does **not**
auto-refresh the `row.project` relationship proxy. If the recipient function then accesses
`provider.row.project`, it sees `None` — silently returning an empty list and skipping
all allocations, with no error.

**Fix:** In recipient functions, when `project_id` may have been set programmatically,
use an explicit session query instead of relationship traversal:

```python
# ❌ WRONG — row.project is None when project_id was set by an early_row_event
def funding_lines_for_charge(provider: LogicRow):
    project = provider.row.project          # None if set programmatically!
    pfd = project.project_funding_definition if project else None
    ...

# ✅ CORRECT — explicit query always works
def funding_lines_for_charge(provider: LogicRow):
    project_id = provider.row.project_id   # read scalar FK (always current)
    if not project_id:
        return []
    project = provider.session.query(models.Project)\
        .filter(models.Project.id == project_id).first()
    if not project or not project.project_funding_definition_id:
        return []
    return provider.session.query(models.ProjectFundingLine)\
        .filter(models.ProjectFundingLine.project_funding_definition_id ==
                project.project_funding_definition_id).all()
```
