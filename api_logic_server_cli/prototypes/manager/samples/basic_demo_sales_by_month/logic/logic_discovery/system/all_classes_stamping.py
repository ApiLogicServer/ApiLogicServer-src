...
def declare_logic():
    ...
    def handle_all(logic_row: LogicRow):  # #als: DATE / USER STAMPING, OPTIMISTIC LOCKING
        ...
        Date/User Stamping - auto-populates audit fields on insert/update:
          ...
          CreatedOnYearMonth    → set on insert, from CreatedOn (eg, "2026-09") - use this
                                   as a composite-FK bucket-key component (eg, a monthly
                                   rollup parent) instead of a project-specific event; see
        ...
        did_stamping = False
        if enable_stamping := True:  # #als: change False to True to enable date/user stamping
            row = logic_row.row
