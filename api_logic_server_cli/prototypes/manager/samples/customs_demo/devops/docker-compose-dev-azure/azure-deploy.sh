...
resourcename="customsdemo"  # lower case, only
resourcegroup="customsdemo_rg"
...
# git clone https://github.com/apilogicserver/customsdemo.git
# cd customsdemo
...
az group create --name customsdemo_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group customsdemo_rg --sku S1 --is-linux
az container create --resource-group customsdemo_rg --name customsdemo --image apilogicserver/customsdemo:latest --dns-name-label customsdemo --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//customsdemo.westus.azurecontainer.io:5656'
