...
changelog:
  ...
  - 2.8 (Feb 23 2026) - Added Method 4: System Creation Services - clean domain project from prompt using Claude + project CE (subsystem_creation.md, RequestObjectPattern.md, logic_bank_api.md); updated welcome.md to surface SCS
...
### Method 4: New Domain Project (System Creation Services)

For real applications (customs, billing, HR) without basic_demo schema contamination:
...
Then open the new project — the **project CE** guides everything from there (SQL DDL → rebuild-from-database → logic → F5).
...
- `starter.sqlite` - One `sys_scalar` table (global settings row); use for new domain projects — no schema contamination, and introduces the sys_scalar pattern for runtime-configurable rates/limits
