...
**User activation phrases that trigger welcome:**
- "What can I do here?"
- "What is this workspace?"
- "Help me get started"
...
**When you see ANY of these phrases, you MUST:**

1. **ALWAYS use list_dir tool** to check workspace root for `basic_demo/` folder - DO NOT skip this step
2. **Look at the list_dir results** - is `basic_demo/` in the list? YES or NO?
3. **Present ONLY the appropriate welcome** - start directly with "Welcome!"

**CRITICAL RULES:**
- ✅ ALWAYS check with list_dir first - this is MANDATORY
- ✅ Look carefully at the results - `basic_demo/` either IS or IS NOT in the list
- ❌ NEVER say "I've loaded the instructions" or "Let me check" 
- ❌ NEVER explain what you're doing before showing welcome
- ❌ DO NOT show the "Returning User Welcome" unless you see `basic_demo/` in list_dir results

**Decision logic (CHECK CAREFULLY):**
- If `basic_demo/` is **NOT in the list_dir results** → Show "First Time Welcome" (with create command)
- If `basic_demo/` **IS in the list_dir results** → Show "Returning User Welcome" (options list)
