...
STEP 4: Follow the System Creation Services workflow from the project CE exactly.
        ⚠️ SCHEMA DESIGN IS LAST — complete ALL pre-DDL analysis before writing any SQL:
           ...
           4d. Type hierarchy scan  — if prompt contains "X are Y" / "X is a type of Y" /
                                      "subtypes": use STI (one table, type discriminator,
                                      nullable subtype cols). Never use joined/concrete CTI.
                                      See project CE "Step 4d" for full pattern.
           Only after 4a/4b/4c/4d are complete: write DDL, run rebuild-from-database, seed, logic.

        ⛔ STEP 8 REQUIREMENTS FILES — MANDATORY, NO EXCEPTIONS:
           For EACH logic file written (`logic/logic_discovery/<use_case_name>.py`),
           IMMEDIATELY also write `<name>/docs/requirements/<use_case_name>/requirements.md`
           containing the verbatim prompt excerpt that drove that use case.
           Do NOT defer this. Do NOT skip it because the project was "just created."
           This is the anchor for logic diagrams and requirements traceability.
           See project CE step 8 for the exact format (YAML front matter + verbatim prompt).
           This is SEPARATE from, and IN ADDITION TO, the Manager-level STEP 5 files below.
...
STEP 5: MANDATORY DOCUMENTATION — NO EXCEPTIONS, do NOT skip, do NOT defer:
   ⛔ Write BOTH files before telling the user the project is done:

   a) `<name>/docs/requirements/readme.md` — provenance:
      - What prompt was used (filename or inline text)
      - Date/time and AI model
      - `genai-logic create` command run
      - Any schema decisions made during 4a-4d analysis

   b) `<name>/docs/requirements/ad-libs.md` — every assumption or guess beyond the prompt:
      - Pre-Coding Analysis (schema decisions, pattern choices, rule types identified)
      - Creation Steps — the actual command sequence run, in order:
          1. genai-logic create --project_name=<name> --db_url=...starter.sqlite
          2. sqlite3 <name>/database/db.sqlite — wrote DDL for N tables
          3. genai-logic rebuild-from-database (from <name>/)
          4. python database/test_data/alp_init.py — seeded N rows
          5. logic/logic_discovery/ — wrote N logic files, N rules total
      - Each ad-lib as 🔴 (needs review) or 🟡 (FYI), with reasoning
      - Anything the prompt did not specify that you decided anyway
      - Summarize in chat as well so user sees it immediately
