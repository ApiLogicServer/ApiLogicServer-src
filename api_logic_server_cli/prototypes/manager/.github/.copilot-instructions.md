...
STEP 1: Ask user for project name if not provided (short, snake_case, e.g. allo_dept_gl)
   ...
   🚨 NAME-COLLISION GUARD — this is a SINGLE shared workspace with many existing
   directories (samples/*, basic_demo/, tests/, etc.). Before proceeding:
   - Check whether `<name>/` already exists at the Manager root, OR whether `<name>`
     matches/overlaps an existing `samples/<name>*` directory (e.g. user says
     "basic_demo" but `samples/basic_demo_sample/` already exists).
   - If a collision/overlap is found, do NOT reuse or edit that existing directory —
     pick a distinct new name (e.g. `genai_demo`, not `basic_demo`) and confirm it
     with the user before STEP 2.
   - `samples/` is READ-ONLY REFERENCE CONTENT. Never run `genai-logic create`,
     `rebuild-from-database`, or edit `models.py`/`logic/` inside any `samples/*`
     directory — even if its name superficially matches the user's request.
