...
resourcename="dev-db"  # lower case, only
resourcegroup="dev-db_rg"
...
# git clone https://github.com/apilogicserver/dev-db.git
# cd dev-db
...
az group create --name dev-db_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group dev-db_rg --sku S1 --is-linux
az container create --resource-group dev-db_rg --name dev-db --image apilogicserver/dev-db:latest --dns-name-label dev-db --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//dev-db.westus.azurecontainer.io:5656'
