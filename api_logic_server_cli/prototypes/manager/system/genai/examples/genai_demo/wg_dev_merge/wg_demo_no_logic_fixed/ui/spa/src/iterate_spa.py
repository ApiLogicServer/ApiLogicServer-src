...
def get_config(project_id: str):
    ...
    url = f'http://localhost:8282/{project_id}/ui/admin/admin.yaml'
...
def iterate(previous : str, prompt : str):
    
    with open(f'./finetuning/{previous}/prompt.txt') as f:
        prev_prompt = f.read()
    with open(f'./finetuning/{previous}/HighLight.tsx') as f:
        prev_resp = f.read()
    ...
    Now, improve the previous response:
    {prompt}
    """
