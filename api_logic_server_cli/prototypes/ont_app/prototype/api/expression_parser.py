    ...
    if data:
        for key, value in data.items():
            if sqltypes and key in sqltypes and isinstance(value, str):
                if sqltypes[key] in [-5,2,4,5,-6]: #BIGINT, TINYINT, INT, SMALLINT, INTEGER - BUG in ONT 
                    data[key] = int(value)
            if sqltypes and sqltypes[key] in [91,93] and isinstance(value, int): #DATE, TIMESTAMP 
                from datetime import datetime  
                fmt_string = "%Y-%m-%d" if sqltypes[key] == 91 else "%Y-%m-%d %H:%M:%S"
                data[key] = datetime.fromtimestamp(value / 1000).strftime(fmt_string)  
    return data
